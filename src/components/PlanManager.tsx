    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 flex flex-col space-y-6 pb-12 duration-500">
      <div className="flex shrink-0 items-center justify-between px-1">
        ...
        <button
          ...
          className={`rounded-2xl p-3 shadow-lg transition-transform active:scale-95 ${hasExercises ? 'bg-indigo-600 text-white' : 'cursor-not-allowed bg-slate-200 text-slate-400 shadow-none'}`}
        >
          <Plus size={24} />
        </button>
      </div>
      ...
      {isAdding ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <button
                      ...
                      disabled={!hasExercises}
                      className={`text-[10px] font-black tracking-tight uppercase ${hasExercises ? 'text-indigo-600' : 'cursor-not-allowed text-slate-300'}`}
                    >
                      + {t.exercise}
                    </button>
                  </div>
                  ...
                  {!hasExercises ? (
                    <div className="space-y-2 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-center">
                      <p className="text-xs font-black tracking-widest text-slate-700 uppercase">
                        {t.no_exercises_plan_title}
                      </p>
                      <p className="text-[11px] font-bold text-slate-400">
                        {t.no_exercises_plan_hint}
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {newPlan.exercises?.map((item: PlanFormExercise, idx: number) => (
                        <div
                          key={item.id}
                          className="group relative space-y-3 rounded-2xl bg-slate-50 p-4"
                        >
                          <button
                            type="button"
                            onClick={() => {
                          ...
                          <div className="space-y-2">
                            {item.sets.map((group: PlanSetGroup, groupIdx: number) => (
                              ...
                              >
                                ...
                                <div className="flex w-16 items-center gap-1 rounded-lg border border-indigo-100 bg-indigo-50/50 px-2 py-1.5">
                          ...
                          <button
                            ...
                            className="text-[10px] font-black tracking-tight text-indigo-600 uppercase"
                          >
                            + {t.add_set}
                          </button>
                        </div>
        ...
        <div className="-mx-4 space-y-8 px-4 pb-24">
          {groupedPlans.upcoming.length > 0 && (
            <div className="space-y-4">
              ...
              >
                ...
                <h3 className="flex-1 text-sm font-bold tracking-widest uppercase">
                  {language === 'zh' ? '未完成' : 'UPCOMING'}
                </h3>
                <span className="mr-1 rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-black">
                  {groupedPlans.upcoming.length}
                </span>
          ...
          {groupedPlans.past.length > 0 && (
            <div className="space-y-4">
              ...
              >
                ...
                <span className="mr-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-black">
                  {groupedPlans.past.length}
                </span>
          ...
          {plans.length === 0 && (
            <div className="rounded-[2.5rem] border border-dashed border-slate-200 bg-white py-16 text-center">
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slate-50">
                <Calendar size={32} className="text-slate-200" />
              </div>
              <p className="font-bold text-slate-400 italic">{t.no_plans}</p>
            </div>
          )}
        </div>
      )}
